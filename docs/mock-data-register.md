...
| `data/corpus/game/dev-placeholder-variants.jsonl` | 2026-07-04 | 30 session-authored English game-feedback texts (12 story + 18 noise) — a MOCK pool proving the generator is domain-neutral for the `game` module; the `game` domain is a switchability demo, not a shipping product | a real game corpus if `game` ever becomes a shipped domain; never demo content |
| `data/corpus/game/generated-placeholder-*.jsonl`, `ground-truth-placeholder-*` | 2026-07-04 | deterministic `Domain:Active=game` compositions from the mock pool (gitignored, regenerable) | regenerated from a real game pool if that domain ships |

...
| T3 | Model-capacity DoS | Unauthenticated `/feedback` `/interpret` `/report` `/health` each trigger model work | **`LlmGate` concurrency shedding + per-call timeout** (this PR) + input caps. The rate limit is a shared-audience ceiling on the proxied browser path (ADR-0025 — all visitors share the proxy egress IP); per-IP only for direct-Funnel callers | Bounded, not eliminated; per-visitor throttling absent behind the proxy |
